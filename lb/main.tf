...
resource "azurerm_public_ip" "public_ip" {
  ...
  name                = "AZ-LB-WAN-${title(var.customer)}-${upper(var.environment)}-${title(var.project)}-${each.key}"
...
resource "azurerm_lb" "public_lb" {
  ...
  name                = "AZ-LB-Public-${title(var.customer)}-${upper(var.environment)}-${title(var.project)}"
  location            = var.location
  resource_group_name = "AZ-RG-${title(var.customer)}-${upper(var.environment)}"
  sku                 = "Standard"
  tags                = var.tag
  dynamic "frontend_ip_configuration" {
    ...
    for_each = { for s in local.lb_flat : s.component => s... if s.public && s.protocol != null }
    content {
      name                 = "AZ-LB-IP-Public-${title(var.customer)}-${upper(var.environment)}-${title(var.project)}-${pub.key}"
...
resource "azurerm_lb" "internal_lb" {
  name                = "AZ-LB-Internal-${title(var.customer)}-${upper(var.environment)}-${title(var.project)}"
  location            = var.location
  resource_group_name = "AZ-RG-${title(var.customer)}-${upper(var.environment)}"
  sku                 = "Standard"
  tags                = var.tag
  dynamic "frontend_ip_configuration" {
    ...
    content {
      name                          = "AZ-LB-IP-Internal-${title(var.customer)}-${upper(var.environment)}-${title(var.project)}-${pub.key}"
...
resource "azurerm_lb_backend_address_pool" "lb_backend_address_pool_public" {
  ...
  name                = "AZ-LB-Backend-Public-${title(var.customer)}-${upper(var.environment)}-${title(var.project)}-${each.value[0].component}"
}
...
resource "azurerm_lb_backend_address_pool" "lb_backend_address_pool_internal" {
  ...
  name                = "AZ-LB-Backend-Internal-${title(var.customer)}-${upper(var.environment)}-${title(var.project)}-${each.value[0].component}"
}
...
resource "azurerm_network_interface_backend_address_pool_association" "network_interface_backend_address_pool_association_public" {
  ...
  ip_configuration_name   = "AZ-LAN-${title(var.customer)}-${upper(var.environment)}-${title(var.project)}-${each.key}"
...
resource "azurerm_lb_probe" "lb_probe_public" {
  ...
  resource_group_name     = "AZ-RG-${title(var.customer)}-${upper(var.environment)}"
  loadbalancer_id         = azurerm_lb.public_lb.id
  name                    = "AZ-LB-Probe-Public-${title(var.customer)}-${upper(var.environment)}-${title(var.project)}-${each.key}"
...
resource "azurerm_lb_probe" "lb_probe_internal" {
  ...
  resource_group_name     = "AZ-RG-${title(var.customer)}-${upper(var.environment)}"
  loadbalancer_id         = azurerm_lb.internal_lb.id
  name                    = "AZ-LB-Probe-Internal-${title(var.customer)}-${upper(var.environment)}-${title(var.project)}-${each.key}"
  protocol                = lower(each.value[0].protocol) == "tcp" ? "tcp" : null
  port                    = each.value[0].backend_port
}
...
resource "azurerm_lb_rule" "lb_rule_public" {
  ...
  resource_group_name            = "AZ-RG-${title(var.customer)}-${upper(var.environment)}"
  loadbalancer_id                = azurerm_lb.public_lb.id
  name                           = "AZ-LB-Rule-Public-${title(var.customer)}-${upper(var.environment)}-${title(var.project)}-${each.key}"
  ...
  backend_address_pool_id        = azurerm_lb_backend_address_pool.lb_backend_address_pool_public[each.value[0].component].id
  frontend_ip_configuration_name = "AZ-LB-IP-Public-${title(var.customer)}-${upper(var.environment)}-${title(var.project)}-${each.value[0].component}"
}
...
resource "azurerm_lb_rule" "lb_rule_internal" {
  ...
  resource_group_name            = "AZ-RG-${title(var.customer)}-${upper(var.environment)}"
  loadbalancer_id                = azurerm_lb.internal_lb.id
  name                           = "AZ-LB-Rule-Internal-${title(var.customer)}-${upper(var.environment)}-${title(var.project)}-${each.key}"
  protocol                       = lower(each.value[0].protocol)
  frontend_port                  = each.value[0].frontend_port
  backend_port                   = each.value[0].backend_port
  probe_id                       = lower(each.value[0].protocol) == "tcp" ? azurerm_lb_probe.lb_probe_internal[each.key].id : null
  backend_address_pool_id        = azurerm_lb_backend_address_pool.lb_backend_address_pool_internal[each.value[0].component].id
  frontend_ip_configuration_name = "AZ-LB-IP-Internal-${title(var.customer)}-${upper(var.environment)}-${title(var.project)}-${each.value[0].component}"
}
...
resource "azurerm_lb_nat_rule" "lb_nat_rule" {
  ...
  resource_group_name            = "AZ-RG-${title(var.customer)}-${upper(var.environment)}"
  loadbalancer_id                = azurerm_lb.public_lb.id
  name                           = "AZ-LB-NAT-Rule-${title(var.customer)}-${upper(var.environment)}-${title(var.project)}-${each.key}"
  protocol                       = lower(each.value[0].protocol)
  frontend_port                  = each.value[0].frontend_port
  backend_port                   = each.value[0].backend_port
  frontend_ip_configuration_name ="AZ-LB-IP-Public-${title(var.customer)}-${upper(var.environment)}-${title(var.project)}-${each.value[0].component}"
